"""
Deep extraction for Tiers B, C, D papers using OpenAI Batch API.

Same extraction approach as extract_tier_a.py but for lower-priority tiers.
Shares the same deep_results/ output directory to avoid duplicates.

Usage:
    python src/extract_tiers_bcd.py prepare          # Build batch JSONL files
    python src/extract_tiers_bcd.py submit           # Submit to OpenAI Batch API
    python src/extract_tiers_bcd.py poll             # Check batch status
    python src/extract_tiers_bcd.py collect          # Download and parse results
    python src/extract_tiers_bcd.py status           # Show pipeline progress
"""
...
PIPELINE_DIR = DATA_DIR / "tiers_bcd_pipeline"
...
def load_bcd_with_pdfs() -> list[dict]:
    """Load Tiers B/C/D papers that have PDFs on disk."""
...
def cmd_prepare(args):
    ...
    papers = load_bcd_with_pdfs()
    print(f"Tiers B/C/D papers with PDFs: {len(papers)}")
    ...
    max_papers = getattr(args, 'max', None)
    if max_papers:
        to_extract = to_extract[:max_papers]
        print(f"  Limited to: {max_papers}")
    ...
    for i, paper in enumerate(to_extract):
        ...
        if current_file is None or current_size + line_bytes > MAX_BATCH_FILE_BYTES:
            ...
            batch_idx += 1
            fname = PIPELINE_DIR / f"batch_bcd_{batch_idx:03d}.jsonl"
    ...
    print(f"Model: {MODEL}")
    print(f"\nSubmit with: python src/extract_tiers_bcd.py submit")


def cmd_submit(args):
    client = OpenAI(timeout=300.0)

    batch_files = sorted(PIPELINE_DIR.glob("batch_bcd_*.jsonl"))
    if not batch_files:
    ...
    print(f"Poll with: python src/extract_tiers_bcd.py poll")
...
def cmd_poll(args):
    ...
    if all_done:
        print("\nAll batches done! Collect with: python src/extract_tiers_bcd.py collect")
    else:
...
def cmd_status(args):
    print("=== Tiers B/C/D Extraction Pipeline Status ===\n")

    pdfs = list(PAPERS_DIR.glob("*.pdf")) if PAPERS_DIR.exists() else []
    print(f"Total PDFs on disk: {len(pdfs)}")

    if PIPELINE_DIR.exists():
        batch_files = list(PIPELINE_DIR.glob("batch_bcd_*.jsonl"))
    ...
    if results:
        ...
        for rf in results:
            try:
                ...
                pass
        print(f"Total results (all tiers): {len(results)} papers, {total_claims:,} claims")
    else:
        print("Results: none yet")


def main():
    parser = argparse.ArgumentParser(description="Tiers B/C/D deep extraction pipeline")
